...
const SessionReview = () => {
    ...
    if (loading) {
        return (
            <div role="status" aria-label={t('sessionReview.loadingLabel')} className="flex flex-col items-center justify-center min-h-screen bg-[#f3f4f8] dark:bg-gray-900 text-[#1b0444] dark:text-gray-100 p-5 text-center font-sans tracking-tight" dir={dir}>
                ...
                <h2 className="text-2xl mt-8 mb-4 font-bold tracking-wider">{t('sessionReview.loadingTitle')}</h2>
                <p className="text-gray-600 max-w-sm leading-relaxed text-lg tracking-wide">{t('sessionReview.loadingDesc')}</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div role="alert" className="flex flex-col items-center justify-center min-h-screen bg-[#f3f4f8] dark:bg-gray-900 text-[#1b0444] dark:text-gray-100 p-5 text-center font-sans tracking-tight" dir={dir}>
                ...
                <h2 className="text-2xl text-red-500 mb-4 font-bold">{t('sessionReview.errorTitle')}</h2>
                <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-md leading-relaxed tracking-wide">{t(`sessionReview.${error}`)}</p>
    ...
    const levelTitle = t(`sessionReview.levels.${result.level}`) || result.level;
        ...
        <div className="flex flex-col items-center w-full min-h-screen bg-[#f3f4f8] dark:bg-gray-900 text-[#1b0444] dark:text-gray-100 pb-12 font-sans tracking-tight relative" dir={dir}>
            <header className="flex w-full pt-6 pb-2 bg-transparent items-center justify-between shrink-0 sticky top-0 z-50 px-6">
                ...
                <div className="flex items-center justify-end w-1/3 gap-2">
                    <button
                        onClick={toggleDark}
                        className="w-8 h-8 flex items-center justify-center rounded-lg border border-[#e0e0e8] dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:text-[#2994f9] transition-colors duration-200"
            ...
            <main className="flex-1 flex flex-col items-center w-full max-w-[800px] mx-auto mt-6 px-4 sm:px-6 md:px-8">
                <h1 className="text-[2rem] font-extrabold mb-3 text-[#1b0444] dark:text-gray-100 tracking-wide text-center w-full">{t('sessionReview.pageTitle')}</h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-2 w-full justify-center">
                    <div className="bg-gradient-to-br from-[#31d4ed] to-[#2994f9] rounded-3xl p-6 text-white text-center shadow-[0_8px_30px_rgba(41,148,249,0.15)] flex flex-col items-center justify-center shrink-0 sm:w-[280px]">
                        ...
                        <h2 className="text-lg text-white/90 mb-2 font-medium">{t('sessionReview.expectedLevel')}</h2>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-[30px] p-6 shadow-[0_8px_20px_rgba(184,184,210,0.27)] dark:shadow-none border border-[#f3f4f8] dark:border-gray-700 flex flex-col justify-center flex-1 relative gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-xl text-[#1b0444] dark:text-gray-100 font-bold m-0">{t('sessionReview.overallFeedback')}</h2>
                        </div>
                        <p className="text-gray-600 dark:text-gray-400 leading-relaxed text-[17px] tracking-wide text-center sm:text-right">{result.level === '-' ? t('sessionReview.noSpeechFeedback') : result.feedback}</p>
                    </div>
                </div>
                ...
                {result.level === "-" ? (
                    <div className="bg-white dark:bg-gray-800 rounded-[30px] py-6 px-6 shadow-[0_8px_20px_rgba(184,184,210,0.27)] dark:shadow-none border border-[#f3f4f8] dark:border-gray-700 mt-4 w-full relative flex flex-col items-center justify-center">
                        <div className="w-[80px] h-[80px] rounded-full bg-[#f4f3fd] dark:bg-gray-700 flex items-center justify-center mb-6 text-gray-500 dark:text-gray-400">
                        ...
                        <h2 className="text-2xl text-[#1b0444] dark:text-gray-100 font-bold mb-3 tracking-wide">{t('sessionReview.noSpeechTitle')}</h2>
                        <p className="text-gray-600 text-lg leading-relaxed max-w-xl mx-auto tracking-wide">{t('sessionReview.noSpeechDesc')}</p>
                    </div>
                ) : result.mistakes && result.mistakes.length > 0 ? (
                    <div className="mt-4 w-full">
                        <h2 className="text-2xl font-bold text-[#1b0444] dark:text-gray-100 mb-6 tracking-wider text-center">{t('sessionReview.mistakesTitle')}</h2>
                                ...
                                <div key={idx} className="bg-white dark:bg-gray-800 rounded-[30px] p-6 shadow-[0_8px_20px_rgba(184,184,210,0.27)] dark:shadow-none border border-[#f3f4f8] dark:border-gray-700 relative overflow-hidden w-full">
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 pt-1 overflow-hidden">
                                            <span className="text-red-500 font-bold text-[15px] mb-2 block tracking-wider">{t('sessionReview.youSaid')}</span>
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 pt-1 overflow-hidden">
                                            <span className="text-emerald-500 font-bold text-[15px] mb-2 block tracking-wider">{t('sessionReview.betterSay')}</span>
                        ...
                        <h2 className="text-2xl text-[#1b0444] dark:text-gray-100 font-bold mb-3 tracking-wide">{t('sessionReview.greatTitle')}</h2>
                        <p className="text-gray-600 dark:text-gray-400 text-lg">{t('sessionReview.greatDesc')}</p>
                    </div>
                )}

                <div className="mt-4 w-full flex justify-center">
                    <button className="bg-gradient-to-r from-[#1567c4] to-[#0d6ed1] text-white border-none rounded-2xl py-4 flex items-center justify-center gap-3 px-10 text-[1.1rem] font-bold cursor-pointer transition-all duration-300 shadow-[0_8px_20px_rgba(21,103,196,0.3)] hover:shadow-[0_12px_25px_rgba(21,103,196,0.4)] hover:-translate-y-1 w-full" onClick={() => navigate('/feedback')}>
                        {t('sessionReview.nextButton')}
